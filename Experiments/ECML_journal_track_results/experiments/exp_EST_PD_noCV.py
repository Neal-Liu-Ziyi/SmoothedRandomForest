"""
SRFnet_OOB per_tree_dim Experiments (No CV Model Selection)

This script runs SRFnet experiments using pre-trained RF10 models with only
the per_tree_dim smoothing mode and both kernel types (normal, hyperbolic_secant).

Key Features:
- Loads pre-trained RF10 models from previous experiments
- Runs only per_tree_dim smoothing mode (no CV model selection)
...
Directory Structure:
    results/
        <data_name>/
            ...
            EST_PD_results_noCV/  # Results for per_tree_dim experiments

Usage:
    python exp_EST_PD_noCV.py
"""
...
def run_single_experiment(data_name, n_obs, r, save_dir, epochs=150):
    ...
    Loads pre-trained RF10 and runs per_tree_dim with both kernels.
    """
    # Check if already completed
    pred_file = f'{save_dir}/{data_name}/EST_PD_predictions_noCV/{data_name}_n{n_obs}_r{r}.csv'
    ...
    # Run per_tree_dim for both kernels
    ...
    for kernel_name, result in [('normal', result_normal), ('hypsec', result_hypsec)]:
        # Predictions
        pred_df[f'srf_{kernel_name}_EST_PD_pred'] = result['pred_calibrated']

        # Uncertainties (calibrated)
        pred_df[f'srf_{kernel_name}_EST_PD_total_std'] = result['total_uncertainty_calibrated']
        pred_df[f'srf_{kernel_name}_EST_PD_noise_free_std'] = result['noise_free_uncertainty_calibrated']

        # Variance components (calibrated)
        pred_df[f'srf_{kernel_name}_EST_PD_intra_var'] = result['intra_var_calibrated']
        pred_df[f'srf_{kernel_name}_EST_PD_inter_var'] = result['inter_var_calibrated']
        pred_df[f'srf_{kernel_name}_EST_PD_model_var'] = result['model_var_calibrated']
    ...
    json_file = f'{save_dir}/{data_name}/EST_PD_results_noCV/{data_name}_n{n_obs}_r{r}.json'
...
def run_experiments(data_name_list,
    ...
    for data_name in data_name_list:
        ...
        os.makedirs(f'{data_dir}/EST_PD_predictions_noCV', exist_ok=True)
        os.makedirs(f'{data_dir}/EST_PD_results_noCV', exist_ok=True)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Run SRFnet per_tree_dim experiments (no CV)')
    ...
    print(f"Predictions saved to: {args.save_dir}/<data_name>/EST_PD_predictions_noCV/")
    print(f"Results saved to: {args.save_dir}/<data_name>/EST_PD_results_noCV/")
    print("=" * 80)
